...
const stepMotorConf_T stepper_conf[NUMBER_OF_STEPPER_MOTORS] = 
{
  ...
  {.dirPin = 3, .stepPin = 4 , .enablePin = 5, .timer = &timerB , .callback = stepper2_timer_cb }
};
...
void Stepper_SetDirection(stepMotor_Id motor, uint8_t dir)
{
  if (motor < NUMBER_OF_STEPPER_MOTORS)
  {
    digitalWrite(stepper_conf[motor].dirPin, dir ? HIGH : LOW);
  }
}

void Stepper_SetEnabled(stepMotor_Id motor, bool enabled)
{
  if (motor < NUMBER_OF_STEPPER_MOTORS)
  {
    digitalWrite(stepper_conf[motor].enablePin, enabled ? LOW : HIGH);
  }
}
